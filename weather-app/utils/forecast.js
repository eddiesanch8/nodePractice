...
const forecast = (latitude, longitude, callback) => {
  ...
  request({ url: url, json: true }, (error, { body }) => {
    if (error) {
      ...
      callback(
        undefined,
        `It is currently: ${body.current.temperature} degrees celsius in ${body.location.region}. It feels like ${body.current.feelslike} degrees fahrenheit and the sky is ${body.current.weather_descriptions}`
      );
    }
  });
};
